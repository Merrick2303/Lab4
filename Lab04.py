...
def jump(t: turtle.Turtle, x, y):
    """Moves the turtle to the given (x, y) location without drawing anything."""
    t.penup()
    t.goto(x, y)
    t.pendown()
...
def draw_jack(t, x, y, radius):
    ...
    eye_size = radius / 4
    draw_eye(t, x - .333 * radius, eye_height,radius/4)
    draw_eye(t, x + .333 * radius, eye_height, 30)  # Right eye
    draw_mouth(t, x- .333*radius, -100, radius)  # Mouth
